        ...
        public override void OnUpdate()
        {
            ...
            if(Svc.Objects.Any(x => x is IBattleChara b && b.CastActionId == 31599))
            {
                ...
                if(HasEffect(Effects.RedRot))
                {
                    ...
                    {
                        if(Conf.EnableOverheadHintsGeneric) Reminder("赤塔の中で外側へ", ImGuiColors.DalamudRed);
                    }
                    else
                    {
                        if(Conf.EnableOverheadHintsGeneric) Reminder("赤塔の中で頭割り", ImGuiColors.DalamudRed);
                    }
                }
                else if(HasEffect(Effects.BlueRot))
                {
                    ...
                    if(HasEffect(Effects.Defamation))
                    {
                        if(Conf.EnableOverheadHintsGeneric) Reminder("青塔の中で外側へ", ImGuiColors.TankBlue);
                    }
                    else
                    {
                        if(Conf.EnableOverheadHintsGeneric) Reminder("青塔の中で頭割り", ImGuiColors.TankBlue);
                    }
                }
                else if(HasEffect(Effects.UpcomingCloseTether, 10f))
                {
                    ...
                    if(counter != 4)
                    {
                        ...
                        var partner = me == null ? null : FakeParty.Get().FirstOrDefault(x => x.Address != me.Address && HasEffect(Effects.UpcomingCloseTether, 10f, x));
                        if(isDefamationRed)
                        {
                            if(Conf.EnableVisualElementsTowers) TowerRed(true);
                            if(Conf.EnableOverheadHintsGeneric) Reminder("赤塔の外側へ", ImGuiColors.DalamudRed);
                        }
                        else
                        {
                            if(Conf.EnableVisualElementsTowers) TowerBlue(true);
                            if(Conf.EnableOverheadHintsGeneric) Reminder("青塔の外側へ", ImGuiColors.TankBlue);
                        }
                        if(partner != null)
                        {
                    ...
                    else
                    {
                        ...
                        {
                            if(Conf.EnableVisualElementsTowers) TowerBlue(true);
                            if(Conf.EnableOverheadHintsGeneric) Reminder("青塔の間で頭割り", ImGuiColors.TankBlue);
                        }
                        else
                        {
                            if(Conf.EnableVisualElementsTowers) TowerRed(true);
                            if(Conf.EnableOverheadHintsGeneric) Reminder("赤塔の間で頭割り", ImGuiColors.DalamudRed);
                        }
                    }
                }
                else if(HasEffect(Effects.UpcomingFarTether, 10))
                {
                    ...
                    {
                        if(Conf.EnableVisualElementsTowers) TowerBlue(true);
                        if(Conf.EnableOverheadHintsGeneric) Reminder("青塔の間で" + (counter == 4 ? "最終頭割り" : "頭割り"), ImGuiColors.TankBlue);
                    }
                    else
                    {
                        if(Conf.EnableVisualElementsTowers) TowerRed(true);
                        if(Conf.EnableOverheadHintsGeneric) Reminder("赤塔の間で" + (counter == 4 ? "最終頭割り" : "頭割り"), ImGuiColors.DalamudRed);
                    }
                }
            }
            else
            {
                ...
                else
                {
                    ...
                    {
                        if(Conf.EnableOverheadHintsTether) Reminder("離れる", ImGuiColors.HealerGreen);
                    }
                    if(HasEffect(Effects.CloseTether) && !Svc.Objects.Any(x => x is IPlayerCharacter pc && HasEffect(Effects.FarTether)))
                    {
                        if(Conf.EnableOverheadHintsTether) Reminder("近づく", ImGuiColors.ParsedBlue);
                    }
                }
            }
        }
